...
import { claudeChat, claudeHealth, claudeSetModel } from "../api/client";
import { useCancellableSend, wasAborted } from "../hooks/useCancellableSend";
import type { ChatMessage, ClaudeHealth } from "../api/client";
...
// Anthropic Claude model presets surfaced in the picker. Default is Opus 4.7
// (the most capable model — see claude-api skill guidance). All four are
// callable with the same code path; only token cost and latency differ.
...
export const CLAUDE_MODEL_PRESETS: ClaudeModelPreset[] = [
  { value: "claude-opus-4-7",   label: "Claude Opus 4.7",   context: "1M",   notes: "Most capable — adaptive thinking, default" },
  { value: "claude-opus-4-6",   label: "Claude Opus 4.6",   context: "1M",   notes: "Previous-gen Opus, supports speed: fast" },
  { value: "claude-sonnet-4-6", label: "Claude Sonnet 4.6", context: "1M",   notes: "Balanced cost + speed, adaptive thinking" },
  { value: "claude-haiku-4-5",  label: "Claude Haiku 4.5",  context: "200K", notes: "Fastest + cheapest" },
];

export const CUSTOM_CLAUDE_MODEL = "__custom_claude__";

interface ClaudeChatProps {
  selectedArea: AreaSelection | null;
  datasets: DatasetInfo[];
  autoLabelSummary?: Record<string, unknown> | null;
  /** When provided, messages scroll + composer portals here.
   * Config header stays inline in the sidebar. */
...
const SUGGESTIONS = [
  ...
  "Audit my pipeline: where am I most likely to mislabel polygons in this scene?",
];

// sessionStorage keys — independent from local + NIM (so all three chats keep
// separate keys, models, and history per browser tab).
const SS_API_KEY = "geoenv.claude.apiKey";
const SS_MODEL = "geoenv.claude.model";
const SS_CUSTOM = "geoenv.claude.customModel";
const SS_MESSAGES = "geoenv.claude.messages";
const SS_INPUT = "geoenv.claude.input";
...
export function ClaudeChat({ selectedArea, datasets, autoLabelSummary, chatBodyPortalTarget }: ClaudeChatProps) {
  ...
  // Messages are the only write big enough to blow the sessionStorage quota
  // — tag them for eviction so a sibling chat panel hitting the wall can
  // reclaim space from the oldest history first.
  useEffect(() => { safeSetItem(SS_MESSAGES, JSON.stringify(messages), { trackForEviction: true }); }, [messages]);
  useEffect(() => { safeSetItem(SS_INPUT, JSON.stringify(input)); }, [input]);
  useEffect(() => { safeSetItem(SS_API_KEY, JSON.stringify(apiKey)); }, [apiKey]);
  useEffect(() => { safeSetItem(SS_MODEL, JSON.stringify(selectedModel)); }, [selectedModel]);
  useEffect(() => { safeSetItem(SS_CUSTOM, JSON.stringify(customModel)); }, [customModel]);
  ...
  const refreshHealth = async () => {
    try {
      const h = await claudeHealth(apiKey || undefined);
      setHealth(h);
      return h;
    } catch {
      return null;
  ...
  // Re-check health when the user types a new key (debounced).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    claudeSetModel(resolvedModel).catch(() => { /* next health refresh will surface */ });
  ...
  const send = async (text?: string) => {
    ...
    const signal = begin();
    try {
      const reply = await claudeChat(next, sceneContext(), {
  ...
  const handleClearChat = () => {
    ...
    if (!window.confirm("Clear the entire Claude conversation? This can't be undone.")) return;
    setMessages([]);
    setInput("");
    setError(null);
    try {
    ...
    <div ref={scrollRef} className="flex flex-col h-full min-h-0 overflow-y-auto pr-1">
      ...
      <div className="flex-shrink-0 mb-3 bg-geo-surface border border-geo-border rounded-lg p-3 space-y-2">
          ...
          <input
            ...
            placeholder={
              health?.api_key_set
                ...
                : "sk-ant-... (grab from console.anthropic.com)"
            }
            autoComplete="off"
            spellCheck={false}
        ...
        <ModelInput
            ...
            suggestions={[
              { value: "claude-opus-4-7", label: "Opus 4.7" }, { value: "claude-sonnet-4-6", label: "Sonnet 4.6" }, { value: "claude-haiku-4-5-20251001", label: "Haiku 4.5" }
            ]}
      ...
      {/* Chat body (messages + clear chip + composer) — portals
          to a floating target when chatBodyPortalTarget is set,
      ...
      <div className="space-y-3">
        {messages.length === 0 && (
          <div className="text-[12px] text-geo-muted">
            <div className="mb-2">
              Ask Claude anything about the current scene. Defaults to Opus 4.7
              with adaptive thinking and prompt caching enabled. Same 7 geo
              tools as Local + NIM Chat.
            </div>
        ...
        {sending && (
          <div className="text-[12px] text-geo-muted italic">Claude is thinking…</div>
        )}
      ...
      {messages.length > 0 && (
        <div className="flex-shrink-0 mt-2 flex justify-end">
          <button
            ...
            title="Clear Claude conversation"
          >
        ...
        <textarea
          ...
          placeholder={
            health?.reachable && health?.auth_ok
              ? "Ask Claude about this scene… (Enter to send, Shift+Enter for newline)"
              : "Paste your Anthropic API key above to enable Claude chat"
          }
